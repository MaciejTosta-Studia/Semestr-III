...
def sjf_scheduler(original_processes):
    processes_copy = sorted(original_processes, key=lambda x: x['arrival_time'])
    processes = deque(sorted(processes_copy, key=lambda x: x['arrival_time']))
    ...
    while len(processes_done) < len(original_processes):
        if processes:
            ...
            processes_queue = deque(sorted(list(processes_queue), key=lambda x: x['burst_time']))
